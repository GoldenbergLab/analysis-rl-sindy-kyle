...
function [lik] = mbmf_lik_sindy(choice1short,choice2short,moneyshort,stateshort,choice10,rew_coef,qval_coef,beta_mb,beta_mf,beta,lambda,stickiness) %inputs from raw data: choice @ stage 1, choice @ stage 2, reward (money), state (to tell if the transition is common or rare)
...
%     second-stage choice?
    ...
    tdQ(1) = rew_coef*Qd(stateshort(i), choice2short(i)) - qval_coef*(Qd(1,choice1short(i))^2); 
 
    %RPE - reward - second-stage value estimate
    tdQ(2) = rew_coef*moneyshort(i) - qval_coef*(Qd(stateshort(i),choice2short(i))^2); 
...
% Put priors on parameters
 ...
 lik= lik+log(pdf('beta',rew_coef,1.1,1.1));
 lik= lik+log(pdf('beta',qval_coef,1.1,1.1));
