functions {
  real update_function(real[,] qval, int start, int end, int[] Tsub, int[,] reward, real[] etap, real[] etan, real[] initmu, real[] noise){
...
parameters {
  ...
  real<lower=0, upper=1> etap[N];
  real<lower=0, upper=1> etan[N];
  real initmu[N];
...
model {
  for (k in 1:N) {
            etap[k] ~ normal(.2, .1)T[0,1];
            etan[k] ~ normal(.2, .1)T[0,1];
            initmu[k] ~ normal(0, .1);
            noise[k] ~ gamma(1, .2);
        }

  target += reduce_sum(update_function, qval, 1, Tsub, reward, etap, etan, initmu, noise);
}
