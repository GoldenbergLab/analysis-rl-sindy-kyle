functions {
  real update_function(real[,] qval, int start, int end, int[] Tsub, int[,] reward, real[] alpha, real[] initmu, real[] noise){
...
model {
  for (k in 1:N) {
            alpha[k] ~ normal(.2, .1)T[0,1];
            initmu[k] ~ normal(0, .1);
            noise[k] ~ gamma(1, .2);
        }

  target += reduce_sum(update_function, qval, 1, Tsub, reward, alpha, initmu, noise);
}
